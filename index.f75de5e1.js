...
for (const inputName of inputNames){
    ...
    const input = inputName === "office" ? document.createElement("select") : document.createElement("input");
    input.name = inputName;
    input.required = true;
    input.dataset.qa = inputName;
    label.textContent = inputName[0].toUpperCase() + inputName.slice(1);
    if (inputName === "salary" || inputName === "age") input.type = "number";
    else if (inputName !== "office") input.type = "text";
    if (inputName === "position") input.required = false;
...
form.addEventListener("submit", (e)=>{
    ...
    if (elements.name.value.length < 4) {
        pushNotification("Invalid input", "Name should have more then 3 letters", "error");
        return;
    }
    ...
    addEmployee(elements.name.value, elements.position.value, elements.office.value, elements.age.value, elements.salary.value);
    pushNotification("Success!", "The employee is successfully added to the tale", "success");
});
table.querySelector("thead").addEventListener("click", (e)=>{
    const target = e.target;
    if (target.tagName !== "TH") return;
    for(let i = 0; i < target.parentElement.children.length; i++)if (target.parentElement.children[i] === target) {
        if (i === sortColumn) isASC = !isASC;
        else isASC = true;
        sortColumn = i;
        break;
    }
...
tableBody.addEventListener("click", (e)=>{
    ...
    if (selectedRow !== target && selectedRow !== null) selectedRow.classList.remove("active");
    selectedRow = target;
